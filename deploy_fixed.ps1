# SkillMatch Deployment Script (Fixed Version)
# This script automates the deployment process for the SkillMatch application

# Stop on any error
$ErrorActionPreference = "Stop"

Write-Host "=== SkillMatch Deployment Script (Fixed) ===" -ForegroundColor Cyan
...
ALLOWED_HOSTS=https://$domain,https://www.$domain,http://localhost:3000,http://localhost:8000
...
# Step 4: Update nginx.conf with the domain
...
# Step 5: Check for SSL certificates
...
if (!(Test-Path -Path $sslFullchain) -or !(Test-Path -Path $sslPrivkey)) {
    ...
    if ($generateSsl -eq "y") {
        ...
        if (!(Get-Command openssl -ErrorAction SilentlyContinue)) {
            Write-Host "OpenSSL is not installed. Please install OpenSSL and try again, or manually add SSL certificates to the ssl directory." -ForegroundColor Yellow
            Write-Host "You can continue deployment without SSL for testing, but it won't work securely in production." -ForegroundColor Yellow
        } else {
...
if ($deploy -eq "y") {
    ...
    Write-Host "Your application should be available at: http://localhost (if running locally)" -ForegroundColor Cyan
    Write-Host "For external access, ensure your domain $domain points to this server's IP address." -ForegroundColor Cyan
} else {
